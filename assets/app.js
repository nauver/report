...
function renderHero(){
  ...
  wrap.innerHTML = data.hero.map((m,i)=>`<article class="metric-card reveal" style="animation-delay:${i*80}ms"><div class="metric-value" data-value="${m.value}">0</div><div class="metric-label">${m.label}</div><p class="metric-detail">${m.detail}</p></article>`).join('');
  wrap.querySelectorAll('.metric-value').forEach(el=>animateNumber(el, Number(el.dataset.value)));
}

function renderChannels(){
  document.getElementById('channelCards').innerHTML = data.channels.map((c,i)=>`<button class="channel-card" aria-expanded="false"><strong>${c.name}</strong><div class="channel-value">${c.value}</div><p><b>${c.trend}</b></p><div class="channel-text">${c.text}</div></button>`).join('');
...
renderHero(); renderChannels(); renderCampaigns(); renderSocial(); renderEvents(); renderLessons();
